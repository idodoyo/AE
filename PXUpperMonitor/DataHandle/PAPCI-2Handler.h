...
public:
	...
	HSBool EffectiveTest( HSChar *pBuffer, HSInt tChannel );	

	HSVoid SetChannelInfo( HSUInt tChannel, CHANNEL_INFO tChannelInfo );

public:
	enum { MAX_AE_CHANNELS = 100, MAX_MSG_LEN = 32768 };

	
	static map< HSUInt, CHANNEL_INFO > mDefaultSeting;
