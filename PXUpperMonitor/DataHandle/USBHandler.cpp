	...
	: mIndexManager( pIndexManager ), mDeviceIndex( tIndex )
{
	...
	CString tEventName;
	tEventName.Format( "%s_CYUSB_IN", this->Name().c_str() );

	mInOvLap.hEvent = CreateEvent( NULL, false, false, tEventName );
}
...
HSBool CUSBHandler::OpenChannel( vector< HSInt > &tChannels, vector< HSInt > *pSampleRates, HSString *pErrorMsg )
{
	...
	{
		for ( HSUInt i = 0; i < 2; i++ )
		{
			mSampleRates[ i ] = ( *pSampleRates )[ i ];
		}
	}
...
HSBool CUSBHandler::ThreadRuning( HSInt tThreadID )
{
	...
	/*
	if ( pInBulkEndPoint == NULL )
	{								
		return HSTrue;		
	}

	UCHAR *pInContext = pInBulkEndPoint->BeginDataXfer( ( HSUChar * )pDataBuffer, tLength, &mInOvLap );
	pInBulkEndPoint->WaitForXfer( &mInOvLap, 1000 );
	pInBulkEndPoint->FinishDataXfer( ( HSUChar * )pDataBuffer, tLength, &mInOvLap, pInContext );

	if ( tLength < tFrameBufferLength )
	{						
		HS_INFO( "Recv Data Length: %d", tLength );
		return HSTrue;		
	}
	*/
	...
	if ( pDataBuffer[ 2 ] == 0x00 )
	{
		...
		//HS_INFO( "Channel 1 Time: %d", ( ( HSUShort )( ( HSUChar )pDataBuffer[ 1 ] << 8 | ( HSUChar )pDataBuffer[ 0 ] ) ) );
	}
	else if ( pDataBuffer[ 2 ] == 0x01 )
	{
		...
		//HS_INFO( "Channel 2 Time: %d", ( ( HSUShort )( ( HSUChar )pDataBuffer[ 1 ] << 8 | ( HSUChar )pDataBuffer[ 0 ] ) ) );
	}
	else
	{
		HS_ERROR( "Lable is %d\n", pDataBuffer[ 2 ] );
...
HSBool CUSBHandler::GetDevice( DEVICE_CH_IDENTIFIER tIdentifier, HSUInt tVersion, HSInt64 tDeviceSavePos, vector< HSInt > &tChannels, vector< IDataHandler * > *pDevices, CIndexManager *pIndexManager, CFileObserver *pFileObserver )
{
