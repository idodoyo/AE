...
void CEasyUSBSettingView::SetCardIdentifier( DEVICE_CH_IDENTIFIER tCardIdentifier )
{
	...
	this->mGridPropterty.RemoveAll();

	this->mGridPropterty.EnableHeaderCtrl( false );
	this->mGridPropterty.EnableDescriptionArea();
	this->mGridPropterty.SetVSDotNetLook();
	this->mGridPropterty.MarkModifiedProperties();
	...
	this->mGridPropterty.AddProperty( pGroup );
...
void CEasyUSBSettingView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_MFCPROPERTYGRID_PROPERTY, mGridPropterty);
}
...
void CEasyUSBSettingView::OnInitialUpdate()
{
	...
	mAutoAlignManager.AddControl( IDC_MFCPROPERTYGRID_PROPERTY, CRect( 0, 0, 0, 100 ) );
}
...
void CEasyUSBSettingView::OnClickedBtnOk()
{
	...
	CMFCPropertyGridProperty* pProp = this->mGridPropterty.FindItemByData( PROPERTY_THRESHOLD_ONE );
	if ( pProp != NULL )
	{
		tStr = pProp->GetValue();
	...
	pProp = this->mGridPropterty.FindItemByData( PROPERTY_THRESHOLD_TWO );
	if ( pProp != NULL )
	{
		tStr = pProp->GetValue();
		std::istringstream Istring( ( LPCSTR )tStr );
	...
	pProp = this->mGridPropterty.FindItemByData( PROPERTY_EZUSB_RATEONE );
	if ( pProp != NULL )
	{
		tStr = pProp->GetValue();
	...
	pProp = this->mGridPropterty.FindItemByData( PROPERTY_EZUSB_RATETWO );
	if ( pProp != NULL )
	{
		tStr = pProp->GetValue();
		std::istringstream Istring( ( LPCSTR )tStr );
