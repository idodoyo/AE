...
void CRelationProperty::RefreshDataSource()
{
	...
	pProp = mPropertyGrid.FindItemByData( PROPERTY_FILL_RECT );	
	...
	/*
	pProp = mPropertyGrid.FindItemByData( PROPERTY_SQUARE_LENGTH );	
	if ( mParent == NULL )
	{				
		pProp->SetValue( 1.0 );
	}
...
HSBool CRelationProperty::IsDataHandlerMachedWith( IDataHandler *pDataHandler, HSUInt tChannel )
{
...
HSVoid CRelationProperty::InitDialog()
{
...
HSVoid CRelationProperty::PropertyChanged( CMFCPropertyGridProperty * pProperty )
{
