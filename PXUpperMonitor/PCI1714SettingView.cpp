...
void CPCI1714SettingView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_MFCPROPERTYGRID_PROPERTY, mGridProperty);
}
...
void CPCI1714SettingView::OnBnClickedBtnOk()
{
	...
	CMFCPropertyGridProperty* pProp = mGridProperty.FindItemByData( PROPERTY_PCI1714_RATE );
	if ( pProp != NULL )
	{
		tStr = pProp->GetValue();
	...
	pProp = mGridProperty.FindItemByData( PROPERTY_THRESHOLD_ONE );
	if ( pProp != NULL )
	{
		tStr = pProp->GetValue();
		std::istringstream  Istring( ( LPCSTR ) tStr );
	...
	pProp = mGridProperty.FindItemByData( PROPERTY_THRESHOLD_TWO );
	if ( pProp != NULL )
	{
		tStr = pProp->GetValue();
		std::istringstream  Istring( ( LPCSTR ) tStr );
	...
	pProp = mGridProperty.FindItemByData( PROPERTY_THRESHOLD_THREE );
	if ( pProp != NULL )
	{
		tStr = pProp->GetValue();
		std::istringstream  Istring( ( LPCSTR ) tStr );
	...
	pProp = mGridProperty.FindItemByData( PROPERTY_THRESHOLD_FOUR );
	if ( pProp != NULL )
	{
		tStr = pProp->GetValue();
		std::istringstream  Istring( ( LPCSTR ) tStr );
...
void CPCI1714SettingView::SetCardIdentifier( DEVICE_CH_IDENTIFIER tCardIdentifier )
{
	...
	this->mGridProperty.RemoveAll();

	this->mGridProperty.EnableHeaderCtrl( FALSE );
	this->mGridProperty.EnableDescriptionArea();
	this->mGridProperty.SetVSDotNetLook();
	this->mGridProperty.MarkModifiedProperties();

	CMFCPropertyGridProperty * pGroup = new CMFCPropertyGridProperty( _T("PXDAQ12204设置" )) ;
	...
	this->mGridProperty.AddProperty( pGroup );
