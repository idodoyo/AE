...
const int WM_VALUE_INCORRCET_MESSAGE = WM_USER + 6;
...
void CPCI2SettingView::SetCardIdentifier( DEVICE_CH_IDENTIFIER tCardIdentifier )
{
	...
	mAutoAlignManager.AddControl( IDC_MFCPROPERTYGRID_PCI2, CRect( 0, 0, 0, 50 ) );
	...
	{
		str.Format( "%dk", pPCI2Handler->mChannels[ 1 ].SampleRate );
	}
	else
	{
		str.Format( "%dM", pPCI2Handler->mChannels[ 1 ].SampleRate / 1000 );
	}
	...
	for ( HSUInt i = 1; i <= pPCI2Handler->mChannels.size() ; i++ )
	{
		...
		pProp->SetValue( pPCI2Handler->mDefaultSeting.ThresholdType == 0 ? _T( "固定" ) : _T( "浮动" ) );
		pProp->AllowEdit( FALSE );
		...
		str.Empty();
		str.Format("%ddB", pPCI2Handler->mChannels[ i ].Threshold );
		pProp->SetValue( str );
		pProp->AllowEdit( false );	
		pSensor->AddSubItem( pProp );	
		...
		str.Empty();
		str.Format("%ddB", pPCI2Handler->mChannels[ i ].ThresholdDeadband );
		pProp->SetValue( str );
		pProp->AllowEdit( false );	
		pSensor->AddSubItem( pProp );
		...
		str.Empty();
		str.Format("%ddB", pPCI2Handler->mChannels[ i ].Gain);
		pProp->SetValue( str );
		pProp->AllowEdit( false );
		pSensor->AddSubItem( pProp );
		...
		str.Empty();
		str.Format("%d", pPCI2Handler->mChannels[ i ].HDT );
		pProp->SetValue( str );
		...
		str.Empty();
		str.Format("%d", pPCI2Handler->mChannels[ i ].PDT );
		pProp->SetValue( str );
		pProp->AllowEdit( true );	
		pSensor->AddSubItem( pProp );
		...
		str.Empty();
		str.Format("%d", pPCI2Handler->mChannels[ i ].HLT );
		pProp->SetValue( str );
		pProp->AllowEdit( true );	
		pSensor->AddSubItem( pProp );
		...
		str.Empty();
		str.Format("%d", pPCI2Handler->mChannels[ i ].MaxDuration );
		pProp->SetValue( str );
		pProp->AllowEdit( true );	
		pSensor->AddSubItem( pProp );
		...
		str.Empty();
		str.Format("%d", pPCI2Handler->mChannels[ i ].PreTrigger );
		pProp->SetValue( str );
		pProp->AllowEdit( true );	
		pSensor->AddSubItem( pProp );
		...
		str.Empty();
		str.Format("%dk", pPCI2Handler->mChannels[ i ].WaveformLength );
		pProp->SetValue( str );
		pProp->AllowEdit( false );	
		pSensor->AddSubItem( pProp );
		...
		str.Empty();
		str.Format("%ddB", pPCI2Handler->mChannels[ i ].AmpGain );
		pProp->SetValue( str );
		pProp->AllowEdit( false );	
		pSensor->AddSubItem( pProp );
		...
		str.Empty();
		str.Format("%d", pPCI2Handler->mChannels[ i ].RMS_ASL_Time );
		pProp->SetValue( str );
		pProp->AllowEdit( true );	
		pSensor->AddSubItem( pProp );
		...
		str.Empty();
		str.Format("%d", pPCI2Handler->mChannels[ i ].CounterThreshold );
		pProp->SetValue( str );
		pProp->AllowEdit( true );	
		pSensor->AddSubItem( pProp );
...
void CPCI2SettingView::OnBnClickedBtnOk()
{
	...
	for ( HSUInt i = 1; i <= ( pPCI2Handler->mChannels.size() ); i++ )
	{
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].ThresholdType );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].Threshold );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].ThresholdDeadband );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].Gain );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].MaxDuration );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].PDT );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].HDT );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].HLT );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].PreTrigger );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].AmpGain );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].WaveformLength );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].RMS_ASL_Time );
		}
		...
		{	
			...
			string.clear();
			string << tStr;
			string >> ( pPCI2Handler->mChannels[ i ].CounterThreshold );
		}
	/*
