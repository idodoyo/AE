...
{	
	...
	CDllLoader *pDllManager = CDllLoader::SharedInstance();
	pDllManager->AddDLL( "PXLicense", "GetLicenseInstance" );	
	if ( !pDllManager->Init() )
	{
		AfxMessageBox( "动态链接库加载错误!" );
		return HSTrue;
	}	
	
	if ( !pDllManager->InstanceWithDLL( mLicense, "PXLicense" ) || mLicense == NULL || !mLicense->Init() )
	{
		AfxMessageBox( "许可证验证失败!" );
		return HSTrue;
	}

	if ( mLicense->TimeUp() )
	{
		AfxMessageBox( "许可证已到期!" );
		return HSTrue;
	}

	mDataExportEnabled = mLicense->DataExportEnabled();

	if ( CWaveGenClient::SharedInstance()->StartServer() )
	{
		CWaveGenClient::SharedInstance()->Init();
	}

	/*
	unsigned char tSerial[ 8 ] = { 0 };
	mLicense->GetSerialNum( tSerial );

	CString tValue;
	tValue.Format( "%02X%02X%02X%02X%02X%02X%02X%02X\n", tSerial[ 0 ], tSerial[ 1 ], tSerial[ 2 ], tSerial[ 3 ], tSerial[ 4 ], tSerial[ 5 ], tSerial[ 6 ], tSerial[ 7 ] );
	OutputDebugStringA( tValue );
	*/
	...
	pDocTemplate = new CMultiDocTemplate(IDR_PXUpperMonitorTYPE_C,
		RUNTIME_CLASS(CUA306SettingDoc),
		RUNTIME_CLASS(CChildFrame), // custom MDI child frame
		RUNTIME_CLASS(CUA306SettingView));
	if (!pDocTemplate)
		return FALSE;
	AddDocTemplate(pDocTemplate);
	mDeviceSettinsDocTemplate[ CARD_UA306 ] = pDocTemplate;
...
HSInt64 CPXUpperMonitorApp::StartTime( HSInt64 tStartTime )
{
	if ( tStartTime > 0 )
	{
...
string CPXUpperMonitorApp::GetLocalTime( HSInt64 tTimeNeed )
{
	char tDateBuf[ 64 ];
...
BOOL CAboutDlg::OnInitDialog()
{
	...
	this->GetDlgItem( IDC_STATIC_DESC )->SetWindowText( "    PXAES声发射系统软件的著作权、"
														"版权和知识产权属于长沙鹏翔电子科技有限公司所有，"
														"并受《中华人民共和国著作权法》、《计算机软件保护条理》、"
														"《知识产权保护条例》和相关国际版权条约、"
														"法律、法规，以及其它知识产权法律和条约的保护。\n\n"
														"    任何单位和个人未经长沙鹏翔电子科技有限公司授权不能使用、修改、再发布本软件的任何部分，"
														"否则将视为非法侵害，我公司保留依法追究其责任的权利，"
														"此条款同样适用于长沙鹏翔电子科技有限公司拥有完全权利的文字、图片、表格等内容。" );
...
void CAboutDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		ShellExecute( NULL, "open", TEXT( "http://www.ndttech.net" ), NULL, NULL, SW_SHOWNORMAL );
	}
