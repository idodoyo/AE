...
BEGIN_MESSAGE_MAP(CArgTable, CDialogEx)
	...
	ON_WM_CONTEXTMENU()	
	ON_WM_DESTROY()
	ON_UPDATE_COMMAND_UI(ID_ARGPOPUP_EXPORTDATA, &CArgTable::OnUpdateArgpopupExportdata)
END_MESSAGE_MAP()
...
HSBool CArgTable::ThreadRuning( HSInt tThreadID )
{
	...
	for ( HSUInt i = 0; i < mChannelArgs.size(); i++ )	
	{
		...
		//HS_INFO( "Channel: %d, Get Arg Num: %d", mChannelArgs[ i ].Channel, mChannelArgs[ i ].ArgList.size() );
	}
...
IGraphicPropertyProtocol * CArgTable::CreatePropertyDlg( CWnd *pParent )
{
	...
	pArgProperty->Create( IDD_NORMALPROPERTY, pParent );
	return pArgProperty;
}
...
void CArgTable::OnUpdateArgpopupExportdata(CCmdUI *pCmdUI)
{
	// TODO: Add your command update UI handler code here
	//pCmdUI->Enable( theApp.DataExportEnabled() );
}
