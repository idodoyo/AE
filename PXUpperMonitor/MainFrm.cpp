...
//	EnablePaneMenu(TRUE, ID_VIEW_CUSTOMIZE, strCustomize, ID_VIEW_TOOLBAR);
	...
	*/
	...
	{		
		string tPath = "";
		if ( tIniConfig.ValueWithKey( "Save File Path", tPath, "DATA FILE" ) )
		{
			this->mWndGraphic.SetDefaultFilePath( tPath.c_str() );
			this->mWndGraphic.SetFilePath( tPath.c_str() );
		}
	}	
...
string CMainFrame::SelectedGraphic()
{
	return mSelGraphicName;
}

void CMainFrame::SetSelectedGraphic( string tGraphic )
{
	mSelGraphicName = tGraphic;
}

string CMainFrame::DragingGraphic()
{
	return mDragingGraphicName;
}

void CMainFrame::SetDragingGraphic( string tDragingGraphic )
{
	mDragingGraphicName = tDragingGraphic;
}
...
void CMainFrame::OnControlStart()
{
			...
			{				
				...
				else
				{
					...
					for ( HSUInt j = 0; j < tChannels.size(); j++ )
					{
						...
						tChannelIdentifier.InitChannel( tChannels[ j ] );
						mDeviceInfo.AddSamplate( pDevices[ i ]->Identifier(), tChannels[ j ], pDevices[ i ]->SampleRate( tChannelIdentifier ) );
	...
	/* restart, short reset linear transfer. */
	if ( GRAPHIC_STOP == mGraphicState )
	{
		ResetAllGraphic();
...
void CMainFrame::OnFileOpen()
{
	...
	mFileObserver->SetFile( ( LPCSTR )tFileDialog.GetPathName(), HSTrue, mDataFileHeader.Size() );
	...
	for ( HSUInt i = 0; i < mDeviceInfo.DeviceCount(); i++ )
	{
		...
		mDeviceManager->AddDevice( tType, tChannels, tSampleRates );
		tDevicesOpenChannels.push_back( tChannels );
	}
...
IGraphicProtocol * CMainFrame::FocusedGraphic()
{
	return mFocusedGrahpic;
}
