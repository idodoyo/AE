...
void CStaticGraphic::SetSeconds( double tTotalSeconds, double tBeginSeconds )
{
	...
	if ( mBeginSeconds < 0.01 && mBeginSeconds > -0.01 && mBeginSeconds + mTotalSeconds < 0.01 && mBeginSeconds + mTotalSeconds > -0.01 )
	{
		mSecondUnit = "us";		
...
{	
	...
	{			
		...
		{			
			int tBypassY = this->DigitWithVoltage( tRect, mDataHandler->BypassVoltage( mChannelIdentifier.ChannelIndex() ), ( tRect.Height() - ( mTopDelta * 2 ) ) / mTotalVoltage );
			if ( tBypassY > mTopDelta && tBypassY < tRect.Height() - mTopDelta )
			{
				CPen tBypassPen( PS_DOT, 1, RGB( 0, 0, 250 ) );	
				mTemplateDC->SelectObject( &tBypassPen );
...
{	
	long tValue = ( long )( ( mBeginVoltage - tVoltage ) * tDigitPerVoltage ) + mTopDelta + tRect.top;
	if ( tValue < mTopDelta )
	{
		tValue = mTopDelta - 1;
	}
	else if ( tValue > tRect.Height() - mTopDelta )
	{
		tValue = tRect.Height() - mTopDelta + 1;
	}	
...
bool CStaticGraphic::ExportDataToFile( CDialogEx *pParent )
{
	CDataExportHelper tDataExport( pParent );
